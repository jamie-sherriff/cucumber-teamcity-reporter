...
class TeamCityReporter extends Formatter {
	...
	logTeamCityEvent(eventName, data) {
		switch (eventName) {
			case 'source':
			case 'attachment':
			...
			case 'test-case-started': //don't need
			case 'test-case-finished':  //don't need
			...
			case 'gherkin-document':
				safeFunctionExec(() => {
					this.logSuiteStart(data);
				});
				break;
			case 'test-run-finished':
				safeFunctionExec(() => {
					this.log(formatString(SUITE_END, this.suiteName));
				});
				break;
			default:
